...
#
# INFO: Adapted for use with cicruitpython and to use one shot conversions
...
_REG_ME = const(0x06)  # Mask enable
...
class INA226:
    ...
    (
        MODE_POWERDOWN,
        MODE_SHUNT_TRIGGERED,
        MODE_BUS_TRIGGERED,
        MODE_SHUNT_BUS_TRIGGERED,
        MODE_ADC_OFF,
        MODE_SHUNT_CONTINUOUS,
        MODE_BUS_CONTINUOUS,
        MODE_SHUNT_BUS_CONTINUOUS,
    ) = (
        _MODE_POWERDOWN,
        _MODE_SHUNT_TRIGGERED,
        _MODE_BUS_TRIGGERED,
        _MODE_SHUNT_BUS_TRIGGERED,
        _MODE_ADC_OFF,
        _MODE_SHUNT_CONTINUOUS,
        _MODE_BUS_CONTINUOUS,
    ...
    def __init__(self, i2c, addr=0x40):
        ...
        self.configure(
            ...
            vshct=_VSHCT_588US,
            mode=_MODE_SHUNT_BUS_TRIGGERED,
        )

        # Default calibration equivalent to your original
        # (0.1 ohm, ~3.6A max expected, rounded current_lsb)
    ...
    def _read_u16(self, reg):
        while not self.i2c.try_lock():
            pass
        self.i2c.writeto_then_readfrom(self.addr, bytes([reg]), self._buf)
        self.i2c.unlock()
    ...
    def calibrate(
        self, *, r_shunt_ohms, max_expected_amps=None, current_lsb_a=None, cal_value=None
    ):
        """
        ...
        if cal_value is not None:
            if current_lsb_a is None or current_lsb_a <= 0:
                raise ValueError(
                    "When cal_value is provided, current_lsb_a must be provided and > 0"
                )
    ...
    def _trigger_oneshot_conversion(self):
        ...
        self._write_u16(_REG_CONFIG, self._config)
