...
class MagnetometerCommands(Enum):
    ...
    ID_METER_PROP = (
        0x01  # See page 4 on the alphalab serial handbook for more information.
    )
    ID_METER_SETT = (
        0x02  # See page 6 on the alphalab serial handbook for more information.
    )
    ID_METER_ADC_SETT = (
        0x1C  # See page 7 on the alphalab serial handbook for more information.
    )
    STREAM_DATA = (
        0x03  # See page 8 on the alphalab serial handbook for more information.
    )
...
class Magnetometer:
    ...
    def read_ascii_stream(self):
        ...
        while read_chunk:
            chunk = self.ser.read_until(
                expected=b"\x08", size=21
            )  # magnetometer sends data in 20-byte chunks

            if chunk:
                # Decoding the ASCII data.
                ascii_response += (
                    chunk.decode("ascii", errors="ignore").strip().replace("\x08", "")
                )
    ...
    def get_value(self, byte_array):
        ...
        sign = (
            -1 if (byte_array[1] & 0x08) else 1
        )  # if 4th msb in byte2 is positive, the value is negative
        decimal_power = (
            byte_array[1] & ~0xF8
        )  # 3 lsb denote power of 10 at decimal place

        raw_value = (
            struct.unpack(">I", byte_array[2:6])[0] & 0xFFFFFFFF
        )  # 32 bits for unsigned integer value of the data point
    ...
    def stream_data(self):
        ...
        while no_ack:
            point = self.ser.read_until(
                b"\x08", size=6
            )  # read until the acknowledgment byte
            if point:
                if point == b"\x08":
                ...
                self.send_command(
                    MagnetometerCommands.STREAM_DATA.value
                )  # request data
                if timeouts <= 0:
    ...
    def get_full_datapoint(self, byte_array):
        ...
        sign = (
            -1 if (byte_array[1] & 0x08) else 1
        )  # if 4th msb is positive, the value is negative
        decimal_power = (
            byte_array[1] & ~0xF8
        )  # 3 lsb denote power of 10 at decimal place

        raw_value = (
            struct.unpack(">I", byte_array[1:5])[0] & 0xFFFFFFFF
        )  # 32 bits for unsigned integer value of the data point
        value = (sign * raw_value) / (10.0**decimal_power)  # converts to signed float32
        return {
            "config": config_info,
...
class DebugMagnetometer:
    def __init__(self):
        pass

    def stream_data(self):
        return [66.036, 383.83, -127.76, 168.6]

    def meter_properties(self):
        return {
            "AVBL_FREQS": "0,1,2,5,10,20,30,40,60,120",
            "BASE_FREQ": "0.500",
            "FIRMWARE": "2.08",
            "MAX_DATA_SETS": "2",
            "METER_NAME": "MR3_GAUSSMETER",
            "POSS_FILE_HEAD": "00000001",
            "TABLE_HEADERS": "Time (s), X Axis (mG), Y Axis (mG), Z Axis (mG), Magnitude "
            "(mG)",
            "TABLE_WIDTH": "75",
            "TWO_DEC_OVERANGE": "1999.99",
        }
